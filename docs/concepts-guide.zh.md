...
技能处理完整的交互生命周期：检测用户意图 → 派遣正确的 agent → 收集结果 → 呈现发现 → 提供下一步。在技能内部，**编排器**（`blueprinting`、`optimizing`、`releasing`）管理多步管线，而**执行器**（`scaffolding`、`authoring`、`auditing`、`testing`）执行专注的任务。子代理是**诊断工具** — 它们完成一项不可编辑的工作（将报告写入 `.bundles-forge/`，不修改现有文件）并返回摘要。
