...
事件包括 `SessionStart`、`PreToolUse`、`PostToolUse`、`Stop`、`SubagentStart`、`FileChanged`、`CwdChanged` 等 25+ 种。支持四种 handler 类型：`command`（shell 命令）、`http`（POST 到 URL）、`prompt`（LLM 评估）和 `agent`（带工具的子代理验证器）。钩子可以拦截操作、注入上下文或触发副作用。定义在 `hooks/hooks.json`、设置文件或技能/代理的 frontmatter 中。
...
技能处理完整的交互生命周期：检测用户意图 → 派遣正确的 agent → 收集结果 → 呈现发现 → 提供下一步。在技能内部，**编排器**（`blueprinting`、`optimizing`、`releasing`）管理多步管线，而**执行器**（`scaffolding`、`authoring`、`auditing`）执行专注的任务。子代理是**诊断工具** — 它们完成一项只读工作并返回报告。
